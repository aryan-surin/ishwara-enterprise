              ...
              <div class="image-wrapper">
                ...
                <div class="image-overlay">
                  <div class="social-links">
                    ...
                    >
                      <svg v-if="social.platform === 'linkedin'" viewBox="0 0 24 24" fill="currentColor" style="width: 20px; height: 20px;">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                      </svg>
                      <svg v-else-if="social.platform === 'twitter'" viewBox="0 0 24 24" fill="currentColor" style="width: 20px; height: 20px;">
                        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                      </svg>
                      <svg v-else-if="social.platform === 'github'" viewBox="0 0 24 24" fill="currentColor" style="width: 20px; height: 20px;">
                      ...
                      <svg v-else-if="social.platform === 'email'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 20px; height: 20px;">
...
/**
 * Debug Function - Check Team Members Visibility
 */
const checkTeamMembersVisibility = () => {
  console.log('Team Members Data:', teamMembers.value);
  console.log('Team Members Count:', teamMembers.value.length);
  
  const memberElements = document.querySelectorAll('.team-members .team-member');
  console.log('Team Member DOM Elements Found:', memberElements.length);
  
  memberElements.forEach((element, index) => {
    const computedStyle = window.getComputedStyle(element);
    console.log(`Member ${index} - Opacity: ${computedStyle.opacity}, Display: ${computedStyle.display}, Visibility: ${computedStyle.visibility}`);
  });
};
...
const initGSAPAnimations = () => {
  // First, force visibility of all team members
  const memberElements = document.querySelectorAll('.team-members .team-member');
  ...
  if (!process.client || !window.gsap) {
    console.warn('GSAP not available, using fallback visibility');
    return;
  }
  ...
  // Ensure cards are visible first as a fallback
  cards.forEach(card => {
  ...
  window.gsap.fromTo(cards, 
    ...
    },
    {
      ...
      scrollTrigger: {
        ...
        onRefresh: () => {
          // Fallback: ensure visibility on refresh
          cards.forEach(card => {
...
onMounted(() => {
  ...
  const ensureVisibility = () => {
    const memberElements = document.querySelectorAll('.team-members .team-member');
    console.log(`Found ${memberElements.length} team member elements`);
    ...
    memberElements.forEach((element, index) => {
      ...
      console.log(`Team member ${index} visibility enforced`);
    });
    ...
    if (container) {
      ...
      console.log('Team members container visibility enforced');
    }
  };
  ...
  setTimeout(() => {
    checkTeamMembersVisibility();
    ensureVisibility(); // Double-check
  }, 100);
  
  // Initialize animations after ensuring visibility
  setTimeout(() => {
    ensureVisibility(); // Triple-check before animations
    initGSAPAnimations();
  }, 200);
...
.team-member {
  ...
  opacity: 1 !important; /* Force visibility - override any GSAP settings */
  transform: translateY(0) !important; /* Reset transform by default */
